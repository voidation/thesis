...
%
%
% Version 1.92, copyright (c) by Artinis Medical Systems http://www.artinis.com
...
%%
% Version history
% 1.92      fix - corrected typo causing error when reading metadata
% 06/10/24  
%
% 1.91      fix - correct reading of date from offline files
% 06/06/24  
%
% 1.90      fix - correct reading of DPF with specific device combinations
% 05/22/24  
%
% 1.89      fix - compatibility with 4.0 multi-device measurements
% 02/07/24  fix - compatbility with template with skipped optodes
%           fix - correct date and time in snirf files
%           fix - no empty columns for TSI and TSI_FF data when mixing
%                 TSI and non-TSI subtemplates
%
% 1.88      fix - handle new metadata introduced by oxyfileconvert
% 11/15/23        
%
% 1.87      fix - Atlasviewer conversion can handle unordered input
% 04/18/23        
%
% v 1.86    fix - correct event reading from oxyproj files
% 03/22/23        
%
% v 1.85    fix - always sort wavelengths and data 
% 03/07/23        for nirs and snirf
%           fix - correct detection of missing template for
%                 multidevice measuements
%
% v 1.84    fix - get correct subtemplate interoptode distance when
% 02/10/23        subtemplate distance differs
%           fix - correct LengthUnit for snirf export
%
% v 1.83    fix - correctly pulling stack value of laser always
% 01/11/23  fix - oxy5 and oxy4 files from oxyproj work (does not assume
%                 oxy3 extension anymore)
%
% v 1.82    fix - corrected overwriting H parameter from oxyproj 
% 04/28/22  fix - correcting H and k parameters when unknown
%
...
if nargin < 5
  metaProj = [];
  fprintf('[Artinis] Thank you for using oxysoft2matlab v1.92 (c) Artinis Medical Systems.\n');
  if nargin < 4
    verbose = true;
    if nargin < 3
      savename = '';
...
if ~isempty(metaProj)
  ...
  if isfield(metaProj, 'optodeParameters') && ~isempty(metaProj.optodeParameters)
    ...
    metaInfo.abs_H = metaProj.optodeParameters.absH;
...
switch(target)
  ...
  case {'snirf'}
    ...
    % metaDataTags
    if isfield(metaInfo, 'Date')
    datetime = split(metaInfo.Date);
    elseif isfield(metaInfo, 'CreateDate')
    datetime = split(metaInfo.CreateDate);
    end
    
    metaDataTags                    = struct;
    metaDataTags.SubjectID          = 'default';
    metaDataTags.MeasurementDate    = datetime(1);
    metaDataTags.MeasurementTime    = datetime(2);
    metaDataTags.LengthUnit         = 'cm';
    metaDataTags.TimeUnit           = 's';
    metaDataTags.FrequencyUnit      = 'unknown';
    nirs_data.metaDataTags          = metaDataTags;%put into nirs structure
    
    
    % Swap wavelengths and data so that wavelenths and data are always in 
    % low-high order for all devices, else we get issues when taking median
    % value for wavelengths

    for i = 1:2:length(Rx_TxwLengths)
        if Rx_TxwLengths(i) > Rx_TxwLengths(i+1)
            rawvals_temp = rawvals(:, i);
            rawvals(:, i) = rawvals(:, i+1);
            rawvals(:, i+1) = rawvals_temp;
        
            Rx_TxwLengths_temp = Rx_TxwLengths(i, :);
            Rx_TxwLengths(i, :) = Rx_TxwLengths(i+1, :);
            Rx_TxwLengths(i+1, :) = Rx_TxwLengths_temp;
        end
    end
